<x-app-layout>
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">Edit Master Status</h4>
                    <p class="card-description"> Update the record details below. </p>
                    <form action="{{ route('mstatus.update', $data->id) }}" method="POST" enctype="multipart/form-data" class="forms-sample">
                        @csrf
                        @method('PUT')
                        ...
                        <a href="{{ route('mstatus.index') }}" class="btn btn-light">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
